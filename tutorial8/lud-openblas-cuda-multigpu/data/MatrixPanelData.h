...
class MatrixPanelData : public htgs::IData {
 public:
  ...
  MatrixPanelData *copy()
  {
    MatrixPanelData *copy = new MatrixPanelData(height, blockSize, panelCol, panelOperatingDiagonal, panelState);
    copy->setMemory(memory);
    copy->setOrigMemory(origMemory);
    copy->setOrigHeight(origHeight);
    copy->setMemoryData(memoryData);
    copy->setCudaMemoryStartAddr(cudaMemoryStartAddr);
    copy->setOriginalOperatingDiagonal(originalOperatingDiagonal);
    copy->setWindowed(windowed);
    return copy;
  }
