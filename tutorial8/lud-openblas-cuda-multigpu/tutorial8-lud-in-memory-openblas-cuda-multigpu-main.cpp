...
//      taskGraph->writeDotToFile("pre-exec-testing-output.dot", DOTGEN_FLAG_SHOW_IN_OUT_TYPES); // | DOTGEN_FLAG_HIDE_MEM_EDGES);
      ...
      taskGraph->writeDotToFile(std::to_string(matrixSize) + "-" + std::to_string(blockSize) + "-end-lud-multigpu-" + std::to_string(numGpus) + "-graph-blockpanel.dot", DOTGEN_COLOR_COMP_TIME | DOTGEN_FLAG_HIDE_MEM_EDGES, std::to_string(matrixSize) + "-" + std::to_string(blockSize) + "-MultiGPU-Block-Panel-LUD", std::to_string(gflops) + " GFLOPS");
