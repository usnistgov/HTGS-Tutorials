...
class MatrixMulRule : public htgs::IRule<MatrixPanelData, MatrixPanelMulData> {
 ...
 public:
  MatrixMulRule(int gridHeight, int gridWidth, int numGpus) :
      gridWidth(gridWidth), gridHeight(gridHeight), numGpus(numGpus)
  {
    factorState = new htgs::StateContainer<bool>*[numGpus];
  ...
  void applyRule(std::shared_ptr<MatrixPanelData> data, size_t pipelineId) override {
    ...
    if (data->getPanelState() == PanelState::ALL_FACTORED)
    {
      ...
      panels[pipelineId]->set(panelColumn, panelColumn, data);

      // Update factor state
      for (int row = diagonalColumn; row < gridHeight; row++)
      {
        factorState[pipelineId]->assign(row, panelColumn, true);
      }
      ...
      {
        if (factorState[pipelineId]->has(diagonalColumn, col))
        {
          auto updatePanel = panels[pipelineId]->get(diagonalColumn, col);
    ...
    {
      factorState[pipelineId]->assign(diagonalColumn, panelColumn, true);
      panels[pipelineId]->set(diagonalColumn, panelColumn, data);

      // Check if the diagonal panel has been factored. If it has then send the matmul
      if (factorState[pipelineId]->has(diagonalColumn, diagonalColumn))
      {
        auto factoredPanel = panels[pipelineId]->get(diagonalColumn, diagonalColumn);
        addResult(new MatrixPanelMulData(factoredPanel, data));
      }

    }

//    std::cout << "Factor State " << pipelineId << std::endl;
//    factorState[pipelineId]->printContents();
//    std::cout << std::endl;
//
//    std::cout << "Received Panels" << pipelineId << std::endl;
//    panels[pipelineId]->printState();
//    std::cout << std::endl;
  }
