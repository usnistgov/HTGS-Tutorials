...
int main(int argc, char *argv[]) {
  ...
  for (int numTry = 0; numTry < numRetry; numTry++) {
    ...
    if (runSequential) {
      ...
      std::cout << " Num bytes available: " << cudaGetFreeBytes(contexts[0]) << std::endl;
      ...
      taskGraph->addCudaMemoryManagerEdge("FactorMem", copyInLower, matrixMulTask, new CudaMatrixAllocator(blockSize, matrixSize), 5, htgs::MMType::Static, contexts);
      taskGraph->addCudaMemoryManagerEdge("UpdateMem", copyInUpper, copyResultBack, new CudaMatrixAllocator(blockSize, matrixSize), 10, htgs::MMType::Static, contexts);
