 ...
 public:
  ...
  virtual void executeGPUTask(std::shared_ptr<MatrixPanelMulData> data, CUstream stream) {
    ...
    cublasDtrsm_v2(handle,
                   ...
                   factored->getMemoryData()->get(), fullMatrixHeightA,
                   update->getMemoryData()->get(), fullMatrixHeightB);


    double *upperFactoredMatrix = update->getMemoryData()->get();

    // Move down 1 block past diagonal
    double *lowerFactoredMatrix = &factored->getMemoryData()->get()[IDX2C(blockSize, 0, fullMatrixHeightA)];

    double *updateMatrix = &update->getMemoryData()->get()[IDX2C(blockSize, 0, fullMatrixHeightB)];
