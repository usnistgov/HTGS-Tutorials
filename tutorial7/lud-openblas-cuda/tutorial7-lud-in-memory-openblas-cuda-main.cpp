...
#include "tasks/MatrixCopyInGausTask.h"
...
int main(int argc, char *argv[]) {
  ...
  for (int numTry = 0; numTry < numRetry; numTry++) {
    ...
    if (runSequential) {
      ...
      int numGpus = 1;
      int *gpuIds = new int[numGpus];
      CUcontext * contexts = initCuda(numGpus, gpuIds);

      MatrixCopyInGausTask *gausCopyInTask = new MatrixCopyInGausTask(blockSize, contexts, gpuIds, numGpus, matrixSize, gridWidth);

      auto gausElimBk = new htgs::Bookkeeper<MatrixBlockMultiData<double *>>();
...
//
      ...
      MatrixCopyInFactorTask *copyInFactorLower = new MatrixCopyInFactorTask(blockSize, contexts, gpuIds, numGpus, matrixSize, gridWidth);
      FactorLowerTask *factorLowerTask = new FactorLowerTask(contexts, gpuIds, numGpus, matrixSize, matrixSize);
      ...
      MatrixCopyOutTask *matrixCopyOutTask = new MatrixCopyOutTask(blockSize, contexts, gpuIds, numGpus);
      ...
      taskGraph->addEdge(gausElimTask, gausCopyInTask);

      taskGraph->addEdge(gausCopyInTask, gausElimBk);
      taskGraph->addRule(gausElimBk, factorUpperTask, gausElimRuleUpper);
      taskGraph->addRule(gausElimBk, copyInFactorLower, gausElimRuleLower);

      taskGraph->addEdge(copyInFactorLower, factorLowerTask);
