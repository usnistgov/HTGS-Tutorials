  ...
  void applyRule(std::shared_ptr<MatrixBlockMultiData<double *>> data, int pipelineId) {
    ...
    else if (dataRow > dataCol)
    {
      ...
      if (!this->matrixMemoryBlocks->has(dataRow, dataCol)) {
        auto storedData = data->getMatrixBlockMemoryData();
        this->matrixMemoryBlocks->set(dataRow, dataCol, storedData);
      }
      ...
      for (int c = dataCol+1; c < gridWidth; c++)
      {
        ...
        if (stateTest == dataState && resultTest == stateTest-1)
        {
          ...
          addResult(new MatrixBlockMulDataPreCopy<double *>(data->getMatrixBlockMemoryData(), matrixUpper, matrixResult));
