...
class UpdateRuleMatMul : public htgs::IRule<MatrixBlockData<double *>, MatrixBlockData<MatrixMemoryData_t>>
{
 public:
...
//    std::cout << "Received " << row << ", " << col << " totalBlocks = " << totalBlocks << std::endl;
    ...
    auto result = new MatrixBlockData<MatrixMemoryData_t>(data->getRequest(), nullptr, 0L, 0L);
      addResult(result);
