...
#ifndef HTGS_TUTORIALS_GAUSELIMRULEUPPERWINDOW_H
#define HTGS_TUTORIALS_GAUSELIMRULEUPPERWINDOW_H

#include <htgs/api/IRule.hpp>
#include "../data/MatrixFactorData.h"
#include "../../../tutorial-utils/util-matrix.h"
#include "../data/MatrixPanelData.h"
class GausElimRuleUpperWindow : public htgs::IRule<MatrixBlockData<double *>, MatrixPanelData>
{

 public:

  GausElimRuleUpperWindow(htgs::StateContainer<std::shared_ptr<MatrixBlockData<double *>>> *matrixBlocks, int gridHeight, int gridWidth, int blockSize, int windowSize) :
      matrixBlocks(matrixBlocks), gridWidth(gridWidth), gridHeight(gridHeight), blockSize(blockSize), windowSize(windowSize)
  {
  ...
  virtual ~GausElimRuleUpperWindow() {
    delete upperState;
  }
    ...
    if (row == col) {
      ...
      for (int c = col + 1; c < gridWidth; c++) {
        ...
        if (nextBlockReady) {
          ...
          if ((panelCol - activeDiagonal) <= windowSize) {
...
//    std::cout << std::endl;
  ...
  std::string getName() {
    return "GausElimRuleUpperWindow";
  }
 private:
  // Store state . . .
...
#endif //HTGS_TUTORIALS_GAUSELIMRULEUPPERWINDOW_H
