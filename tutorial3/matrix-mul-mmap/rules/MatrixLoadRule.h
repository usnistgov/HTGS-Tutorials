...
class MatrixLoadRule : public htgs::IRule<MatrixBlockData<double *>, MatrixBlockMulData<double *>> {
...
//                  << "Updating C[" << rowB << "](" << rowA << ", " << colB << ")" << std::endl;
              // Schedule work
              addResult(new MatrixBlockMulData<double *>(data, matrixBState->get(rowB, colB)));
...
//                  << "Updating C[" << colA << "](" << rowA << ", " << colB << ")" << std::endl;
              // Schedule work
              addResult(new MatrixBlockMulData<double *>(matrixAState->get(rowA, colA), data));
              MatrixState state = MatrixState::IN_FLIGHT;
              container->set(rowA, colB, state);
            }
          }
...
//    }
 ...
 private:
  ...
  int blockWidthB;
  int blockHeightB;
  htgs::StateContainer<std::shared_ptr<MatrixBlockData<double *>>> *matrixAState;
  htgs::StateContainer<std::shared_ptr<MatrixBlockData<double *>>> *matrixBState;
